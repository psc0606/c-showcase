...
int main(int argc, char *argv[])
{
	...
	is_loopback = Ad9361ReadReg(via_fpga, fd_spi, 0x3f5);
	}
	while (1) {
		...
		break;
		/*case 'a'://rad timer
			ret = ioctl(fd_rad, TRAD_IOCTL_TM_REGS, &radtmregs);
			print_tm_regs(&radtmregs);
		break;*/
		case 'a'://print 9361 regs
			if(whereami == 0)
				Ad9361DumpRegs(via_fpga, fd_spi);
			else
		...
		case 'i'://loopback
			if(is_loopback){
				...
				Ad9361WriteReg(via_fpga, fd_spi, 0x3f5, 0x00);
				Ad9361WriteReg(via_fpga, fd_spi, 0x02, 0x0);
				is_loopback = 0;
			}else{
				//ret = ioctl(fd_rad,TRAD_IOCTL_JDMA_CONTROL ,&jdma_handle);
				Ad9361WriteReg(via_fpga, fd_spi, 0x3f5, 0x01);
				Ad9361WriteReg(via_fpga, fd_spi, 0x02, 0xde);
				is_loopback = 1;
			}
		break;
