...
WINDOW *g_win = NULL;
GRAPHICS_COLOR g_fore_color = GCCyan;
GRAPHICS_COLOR g_back_color = GCRed;
...
bool init_graphics(void)
{
	...
	cbreak();	//控制读取出ctrl以外的字元
	noecho();	//控制输入字符不在终端上显示
	
	//keypad(g_win,TRUE);	启用curses中对键盘特殊键的定义
...
int read_key()
{
	...
	if (ch == 27)
	{
		...
		//fcntl():stdin_fileno代表要设置的文件。F_GETFL取得文件描述词状态旗标，来自open()中的flag
