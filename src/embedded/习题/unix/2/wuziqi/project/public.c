...
void huifang_jilu(Node seat[290],int n)
{
	FILE *fp;
	...
	while (1)
	{
		...
		fwrite(&seat[i++],sizeof(Node),1,fp);
	}
	fclose(fp);
}

void huifang(int i,int j)
{
	init_background();
	Node b;
	FILE *fp;
	...
	table_qipan(i);														//初始化棋盘
	table_player(i,j);													//选择模式后画出棋盘
	while (1)
	{
		if (fread(&b,sizeof(Node),1,fp) <= 0)
		{
			break;
		}
		set_chess_huifang(b.i,b.j,n++);
	}
	fclose(fp);
	while (1)
	{
...
void set_chess_huifang(int i, int j, int n)		//落子！
{
	if (n % 2 == 1)
	{
...
void people(int a[19][19],int k,Node seat[290],int *n)	
{
	int ch, m;
	int i = 9, j = 9;
...
void computer_chess(int j)
{
	...
	computer(a,j,seat,&n);								//移动光标
	huifang_jilu(seat,n);
}

void computer(int a[19][19],int k,Node seat[290],int *n)		//移动光标,k == 1，选手为白棋
{
	int ch, m;
	int i = 9, j = 9, x1, y1;
