...
#define JDMA_MAP1_TX_BUF_BASEADDR				(JDMA_MAP0_TX_BUF_BASEADDR + TRANSMIT_PER_SUBFRAME_20MHZ*10)
#define JDMA0_SAMPLING_10SUBFRAME_DATA_BASEADDR		(JDMA_MAP1_TX_BUF_BASEADDR + TRANSMIT_PER_SUBFRAME_20MHZ*10)
#define JDMA1_SAMPLING_10SUBFRAME_DATA_BASEADDR		(JDMA0_SAMPLING_10SUBFRAME_DATA_BASEADDR + JDMA_10MS_SUBFRAME_DATA_SIZE)
#define MAP0_OUT_BOUND_10SUBFRAME_DATA_BASEADDR		(JDMA1_SAMPLING_10SUBFRAME_DATA_BASEADDR + JDMA_10MS_SUBFRAME_DATA_SIZE)
#define MAP1_OUT_BOUND_10SUBFRAME_DATA_BASEADDR		(MAP0_OUT_BOUND_10SUBFRAME_DATA_BASEADDR + MAP_10MS_SUBFRAME_DATA_SIZE)

//10 subframs, 10ms data
#define JDMA_MAP0_TX_BUF_SIZE					(TRANSMIT_PER_SUBFRAME_20MHZ*10)
#define JDMA_MAP1_TX_BUF_SIZE					(TRANSMIT_PER_SUBFRAME_20MHZ*10)
#define JDMA_5MS_SUBFRAME_DATA_SIZE				(TRANSMIT_PER_SUBFRAME_20MHZ  * 5)
#define JDMA_10MS_SUBFRAME_DATA_SIZE			(TRANSMIT_PER_SUBFRAME_20MHZ  * 10)
#define MAP_10MS_SUBFRAME_DATA_SIZE				(TRAMSMIT_PER_SUBFRAME_MAP_20MHZ * 10)
...
static void RadDrvJdmaSetup(UINT32 RadId, UINT32 NumAntennas)
{
    ...
    /*
     * JDMA1 MAP PORT 0的Tx Template0,每个Tsl将Tx buf中的1个subframe发送出??? */
    ...
    /*
     * JDMA1 MAP PORT 0的Rx Template1,每个Tsl???个subframe接收
     * 小区搜索阶段使用这个Template
     */
    ...
    /*
     * JDMA1 MAP PORT 1的Tx Template0,每个Tsl将Tx buf中的1个subframe发送出???     */
    ...
    TslEntry0 = (0<<7) | (0<<4) | (0<<3) | (0<<2) | (1<<1) | 1; //Use template 0,1 subframe,repeat
	TslEntry1 = TslEntry0; 
	TslEntry2 = TslEntry0; 
	TslEntry3 = TslEntry0; 
	TslEntry4 = TslEntry0; 
	...
	TslEntry7 = TslEntry0; 
	TslEntry8 = TslEntry0; 
	TslEntry9 = (0<<7) | (0<<4) | (0<<3) | (1<<2) | (1<<1) | 1; 
	pTxSymDesc->TslEntry[0] = (TslEntry3 << 24) | (TslEntry2 << 16) | (TslEntry1 << 8) | TslEntry0; 
	pTxSymDesc->TslEntry[1] = (TslEntry7 << 24) | (TslEntry6 << 16) | (TslEntry5 << 8) | TslEntry4; 
	pTxSymDesc->TslEntry[2] = (TslEntry9 << 8) | TslEntry8;
...
static void RadDrvEnable(int RadId)
{
    ...
    loval = 0x1f;
    //hival = 0x77f;
		hival = (pRadDrvCtx->SamplingRate)/ (loval + 1) - 1;//??REG32(RAD_CFG_BASEADDR + 0x184) = 1;//0????
    /*
     * syncnet
     */
    ...
    //REG32(TRANSCEDE_GPIO_OUTPUT_REG) |= GPIO_0; //toggle radio state (move to ACTIVE state),这个GPIO的使能对Slave无效,被FPGA屏蔽???    REG32(TRANSCEDE_GPIO_OUTPUT_REG) &= ~GPIO_0;
...
int tradio_ioctl(int DevId, unsigned int cmd, unsigned long arg)
{
	...
	for(i = 0; i < 10; i++) {
		...
		if(0 == i%2)
			pMdma_ctrl_regs0->DMAFcomCtrl.HeadFDesc = (unsigned int)pMdma_Desc_phys;//??ͬһ??mdma?п??ܻ??г?ͻ??ץ????????
		else
