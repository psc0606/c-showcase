...
void Ad9361SetLoopback(int via_fpga, int fd, uint32_t Loopback)
{
    if (Loopback)
    {
        //
        ...
        //
        Ad9361WriteReg(via_fpga, fd, 0x3f5, 0x01);
    ...
    else
    {
        ...
        //
        Ad9361WriteReg(via_fpga, fd, 0x3f5, 0x00);
...
unsigned int Ad9361ReadReg(int via_fpga, int fd, unsigned short addr)
{
    unsigned int val;
	if(1 == via_fpga)
		val = read_spi(fd, addr);
	else
...
void Ad9361DumpRegs(int via_fpga, int fd)
{
    unsigned int addr;
    unsigned int val;
    ...
    for (addr = 0; addr < 0x400; addr++)
    {
        ...
        val = Ad9361ReadReg(via_fpga, fd, addr);
        printf("%02x ", val);
    }
    printf("\n\n");
}
